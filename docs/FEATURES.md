...
| 功能 | 原版 | CF | 说明 |
|------|------|----|------|
| 首页（排行 / 最新 / 搜索框） | ✅ | ✅ | DOM/CSS 对齐原版；仅「最新」时居中；榜单最多 10 条；**最新/分类列表置顶优先**（不显示「顶」标记） |
| 本地搜索列表 `/s/关键词.html` | ✅ | ✅ | 侧栏热榜、筛选、高亮、分页；**置顶优先**（不显示「顶」标记）；正式资源走 **KV 精简索引**；索引脏时回退 D1，置顶后异步重建 |
| 资源详情 `/d/id.html` | ✅ | ✅ | 相关资源 + 侧栏热榜 |
| 搜索模式：精准 / 模糊 / 分词 | ✅ | ✅ | |
| 关键词屏蔽 | ✅ | ✅ | el-alert 违规提示 |
| 全网搜 SSE（api/html/tg/kk） | ✅ | ✅ | Qloading / Ebox / btns2；**`scene`**：资源/音乐分线路，`?scene=0|1` |
...
| | 导出 / 表格导入 / 批量导入 | ✅ | ✅ | 含「名称链接入库」：不校验、名称+链接一行一条 |
| | 分类全部转存 | ✅ | ✅ |
| **资源 → 分类管理** | 增改删、开关（本地/日更/前台）、图标 | ✅ | ✅ |
| **资源 → 接口配置** | 线路 CRUD、开关、试搜 | ✅ | ✅ | 场景「资源/音乐」；夸克见 [QUARK-SEARCH-LINES.md](./QUARK-SEARCH-LINES.md)；音乐见 [MUSIC-SEARCH-LINES.md](./MUSIC-SEARCH-LINES.md) |
